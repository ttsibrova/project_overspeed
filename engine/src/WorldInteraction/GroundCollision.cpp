...
std::vector <std::pair <geom::Point2D, geom::Point2D>> GetTileLines (geom::Quadrant quadrant,
...
{
    return p.X() >= std::min (p1.X(), p2.X()) - precision::half_pixel && p.X() <= std::max (p1.X(), p2.X()) + precision::half_pixel &&
           p.Y() >= std::min (p1.Y(), p2.Y()) - precision::half_pixel && p.Y() <= std::max (p1.Y(), p2.Y()) + precision::half_pixel;

}


std::optional <geom::Vector2D> SweepCollision (const physics::Collider& playerCollider,
                                               ...
                                               geom::Point2D tileMaxCorner)
{
    ...
    auto tileLines = GetTileLines (geom::getInversedQuadrant (geom::getVectorQudrant (playerTrsl)), tileMinCorner, tileMaxCorner);
    ...
    for (const auto line: tileLines) {
        ...
        if (!newTrsl.has_value() && intersectPnt.has_value()) {
            newTrsl = geom::Vector2D (intersectPnt.value().X() - colliderKeyPnt.X(), intersectPnt.value().Y() - colliderKeyPnt.Y());
        }
        if (newTrsl.has_value() && intersectPnt.has_value()) {
            auto candidate = geom::Vector2D (intersectPnt.value().X() - colliderKeyPnt.X(), intersectPnt.value().Y() - colliderKeyPnt.Y());
...
std::optional <geom::Vector2D> HitScanGround (const physics::Collider& playerCollider, const geom::Vector2D& playerTrsl, const world::GroundData& groundData)
{
    ...
    for (size_t i = left_tile; i <= right_tile; i++) {
        for (size_t j = top_tile; j <= bottom_tile; j++) {
            if (groundData.tiles[std::array{i, j}] != 0) {
                geom::Point2D tileMinCorner (static_cast <float> (i) * groundData.tile_width, static_cast <float> (j) * groundData.tile_height);
                geom::Point2D tileMaxCorner (tileMinCorner.X() + groundData.tile_width, tileMinCorner.Y() + groundData.tile_height);
...
std::optional<geom::Vector2D> GetGroundNormalUnderPlayer (const physics::Collider& playerCollider,
                                                          Quadrant playerVelocityQuadrant,
